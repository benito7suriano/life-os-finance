// System prompt for the Telegram wealth-manager agent. Keep the static text
// first so the provider's prefix cache can reuse it; today's date goes last.
...
- For anything tabular (monthly income, category breakdowns, lists of transactions) use a monospace table inside <pre></pre>: fixed-width columns, numbers right-aligned, exactly one line per row (never wrap a cell), at most ~8 rows unless the client asked for more, and never wider than ~36 characters per line — abbreviate names to fit.
...
/** Static prompt first (Gemini implicit caching keys on the prefix), then the
 * volatile date line. */
export function buildSystemPrompt(now: Date, timeZone: string): string {
  ...
  return `${STATIC_SYSTEM_PROMPT}\n\nToday is ${weekday}, ${today} (${timeZone}). Dates in tool calls are YYYY-MM-DD.`
}
